...
@Component({
  ...
  template: `
    ...
    <style>
      ...
      /* Celular por defecto */
      .main-layout { display: flex; padding: 20px; gap: 20px; flex-direction: column; }

      /* Computadora (Pantallas mayores a 768px) */
      ...
      @media (min-width: 1100px) { .grid-lotes { grid-template-columns: repeat(3, 1fr); } }

      .lote-card { background: white; padding: 20px; border-radius: 12px; cursor: pointer; transition: transform 0.2s; }
      .lote-card:hover { transform: translateY(-5px); }
      .lote-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px; }
      ...
      .search-bar input { width: 100%; padding: 15px; border-radius: 12px; border: 1px solid #ddd; margin-bottom: 25px; font-size: 1rem; box-sizing: border-box; }
